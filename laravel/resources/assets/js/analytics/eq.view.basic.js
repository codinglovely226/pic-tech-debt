;(function ($, window) {
    ...
    EQ.view.basic = {
        ...
        init: function (options) {
            ...
            if (!this._funcs.loadQueryButtonClick) {
                this._funcs.loadQueryButtonClick = function () {
                    $("#loading_wrap").show();
                    f = EQ.client.getQuery();
                    $.post(window.easyQuerySettings.loadQueryUrl, {
                        id: "LastQuery",
                        ...
                        queries.forEach(query => {
                            if (query.statement.trim() == '') return;
                            var radioItem = `<div class="radio"><label><input type="radio" name="loadqueryRadios" id="" value="` + query.filename + `">` + query.filename + `</label></div>`
                            modalBody += radioItem;
                        });
                        if (!modalBody) {
                            alert("Couldn't find stored query")
                            return
                        }
                        $(".loadqueryModalBody").html(modalBody);
                        $("#loadqueryModal").modal('show');
                        $(".loadquery-select").on("click", function () {
                            $("#loadqueryModal").modal('hide');
                            var selFileName = $("input[name='loadqueryRadios']:checked").val();
                        ...
                        $("#loading_wrap").hide()
                        if (error == 'Internal Server Error' && (xhr.responseJSON) && (xhr.responseJSON.result == 'Not Found Data')) {
                            alert("Couldn't find stored query")
                        } else {
            ...
            if (!this._funcs.saveQueryButtonClick) {
                this._funcs.saveQueryButtonClick = function () {
                    ...
                    $(".savequery-select").on("click", function () {
						$("#savequeryModal").modal('hide');
						
						var name = $('#savequeryModal #query-name').val();
						if(name.trim() == '' ){
        ...
        buildAndExecute: function (options) {
            ...
            var requestParams = {
                ...
                success: function (result) {
                    try {
                        ...
                        //result count
                        if (typeof (result.resultCount) === "undefined") {
                            result.resultCount = EQ.core.getText("MsgResultCount").replace("{0}", "0");
                        }
                ...
                error: function (statusCode, errorMessage, operation) {
                    ...
                    self._resultCountSpan.text(EQ.core.getText("MsgResultCount").replace("{0}", "0"));
                    self._resultCountSpan.show();

                    var processed = false;
                    if (self.errorCallback) {
                        processed = self.errorCallback(statusCode, errorMessage, operation);
                    }
            ...
            if (options.queryId) {
                requestParams["queryId"] = options.queryId;
                if (options.modelId) {
                    requestParams["modelId"] = options.modelId;
                }
            }
            else {
                requestParams["query"] = EQ.client.getQuery();;
            }

            EQ.client.buildAndExecute(requestParams);
        }
    };

    EQ.view.init = function() {
		EQ.view.basic.init();
	}
