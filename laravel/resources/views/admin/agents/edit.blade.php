    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.agents.update", [$agent->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent_reference_name') ? 'is-invalid' : '' }}" type="text" name="agent_reference_name" id="agent_reference_name" value="{{ old('agent_reference_name', $agent->agent_reference_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent_reference_url') ? 'is-invalid' : '' }}" type="text" name="agent_reference_url" id="agent_reference_url" value="{{ old('agent_reference_url', $agent->agent_reference_url) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agent_reference_source') ? 'is-invalid' : '' }}" type="text" name="agent_reference_source" id="agent_reference_source" value="{{ old('agent_reference_source', $agent->agent_reference_source) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dataset') ? 'is-invalid' : '' }}" name="dataset_id" id="dataset_id">
                    @foreach($datasets as $id => $dataset)
                        <option value="{{ $id }}" {{ ($agent->dataset ? $agent->dataset->id : old('dataset_id')) == $id ? 'selected' : '' }}>{{ $dataset }}</option>
                    @endforeach
                </select>
