...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.siteDataSet.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.site-data-sets.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('datasets') ? 'is-invalid' : '' }}" name="datasets[]" id="datasets" multiple required>
                    @foreach($datasets as $id => $dataset)
                        <option value="{{ $id }}" {{ in_array($id, old('datasets', [])) ? 'selected' : '' }}>{{ $dataset }}</option>
                    @endforeach
                </select>
