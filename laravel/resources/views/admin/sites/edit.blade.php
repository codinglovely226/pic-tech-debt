    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sites.update", [$site->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('site_name') ? 'is-invalid' : '' }}" type="text" name="site_name" id="site_name" value="{{ old('site_name', $site->site_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shortcode') ? 'is-invalid' : '' }}" type="text" name="shortcode" id="shortcode" value="{{ old('shortcode', $site->shortcode) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('site_default_time_zone') ? 'is-invalid' : '' }}" type="number" name="site_default_time_zone" id="site_default_time_zone" value="{{ old('site_default_time_zone', $site->site_default_time_zone) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organisation') ? 'is-invalid' : '' }}" name="organisation_id" id="organisation_id">
                    @foreach($organisations as $id => $organisation)
                        <option value="{{ $id }}" {{ ($site->organisation ? $site->organisation->id : old('organisation_id')) == $id ? 'selected' : '' }}>{{ $organisation }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country_id" id="country_id" required>
                    @foreach($countries as $id => $country)
                        <option value="{{ $id }}" {{ ($site->country ? $site->country->id : old('country_id')) == $id ? 'selected' : '' }}>{{ $country }}</option>
                    @endforeach
                </select>
