...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-siteSpecialists">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($specialists as $key => $specialist)
                        <tr data-entry-id="{{ $specialist->id }}">
                            ...
                            <td>
                                @foreach($specialist->sites as $key => $item)
                                    <span class="badge badge-info">{{ $item->site_name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('specialist_show')
...
@endcan
  ...
  let table = $('.datatable-siteSpecialists:not(.ajaxTable)').DataTable({ buttons: dtButtons })
