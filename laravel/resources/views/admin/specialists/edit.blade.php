    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.specialists.update", [$specialist->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('specialist_name') ? 'is-invalid' : '' }}" type="text" name="specialist_name" id="specialist_name" value="{{ old('specialist_name', $specialist->specialist_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('specialty') ? 'is-invalid' : '' }}" type="text" name="specialty" id="specialty" value="{{ old('specialty', $specialist->specialty) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('user_in_system') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="user_in_system" id="user_in_system" value="1" {{ $specialist->user_in_system || old('user_in_system', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($specialist->user ? $specialist->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sites') ? 'is-invalid' : '' }}" name="sites[]" id="sites" multiple>
                    @foreach($sites as $id => $site)
                        <option value="{{ $id }}" {{ (in_array($id, old('sites', [])) || $specialist->sites->contains($id)) ? 'selected' : '' }}>{{ $site }}</option>
                    @endforeach
                </select>
                @if($errors->has('sites'))
                    <div class="invalid-feedback">
