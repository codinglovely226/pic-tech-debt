    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscriptions.update", [$subscription->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('organisation') ? 'is-invalid' : '' }}" name="organisation_id" id="organisation_id" required>
                    @foreach($organisations as $id => $organisation)
                        <option value="{{ $id }}" {{ ($subscription->organisation ? $subscription->organisation->id : old('organisation_id')) == $id ? 'selected' : '' }}>{{ $organisation }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('subscription_type') ? 'is-invalid' : '' }}" name="subscription_type" id="subscription_type" required>
                    ...
                    @foreach(App\Subscription::SUBSCRIPTION_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('subscription_type', $subscription->subscription_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('subscription_start') ? 'is-invalid' : '' }}" type="text" name="subscription_start" id="subscription_start" value="{{ old('subscription_start', $subscription->subscription_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', $subscription->duration) }}" step="1" required>
