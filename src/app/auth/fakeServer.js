...
const server = {
  ...
  init() {
    if (localStorage.users === undefined || !localStorage.encrypted) {
      // Set default user
      const admin = 'admin';
      const adminSalt = genSalt(admin);
      const adminPass = hashSync('password', adminSalt);

      const user = 'user';
      const userSalt = genSalt(user);
      const userPass = hashSync('user', userSalt);

      users = {
        [admin]: hashSync(adminPass, salt),
        [user]: hashSync(userPass, salt),
      };
